...
<div class="card h-100 shadow-sm border-0">
    <a href="{{ route('customer.products.show', $product) }}"
       class="ratio ratio-4x3 bg-light d-block text-decoration-none rounded-top overflow-hidden">
        ...
        @else
            <span class="d-flex align-items-center justify-content-center text-muted small">
                {{ __('No image') }}
            </span>
        @endif
    </a>
    ...
    <div class="card-body d-flex flex-column">
        <h2 class="h6 mb-1">
            <a href="{{ route('customer.products.show', $product) }}"
               class="text-decoration-none text-reset stretched-link-none">
                {{ $product->name }}
            </a>
        </h2>
        ...
        <div class="mt-auto">
            <p class="h5 mb-2">€{{ number_format((float) $product->price, 2) }}</p>
            <p class="mb-3"><x-stock-badge :product="$product" /></p>
